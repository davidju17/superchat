...
public class ChatServerThread implements Runnable
{
   ...
   public void run()
   {
      ...
      {
         // This is only thrown is when closing a socket and thread is about to
...
//            break;
   ...
   // Used for:
   // - broadcasting identitychange of a current user to all users (loop through
   // clientList).
   ...
   private synchronized void IdentityChange(JSONObject jsonObjRec)
   {
      ...
      // Checks if proposed identity is valid. If not it simply returns the old
      // name.
          ...
          newIdentity.length() > 2 && newIdentity.length() < 17)
      {
         ...
         for (int i = 0; i < ChatServerRoom.getRoomCount(); i++)
         {
            ...
            if (roomCheck.getOwnerId()
                     .equals(this.identity) && !roomCheck.getOwnerId()
                     .equals(""))
            {
               ChatServerMain.getRoom(i).setOwnerId(jsonObjRec.get("identity")
                        .toString());
            }
   ...
   // intiator. OK
   // - initial 'change' to MainHall when first joining (can treat same as above
   // case?)
   ...
   public synchronized void RoomChange(JSONObject jsonObjRec)
   {
      ...
      if (!(requestedRoom.equals("MainHall") || requestedRoom.equals("")))
      {
         for (int a = 0; a < banRoom.size(); a++)
         {
            ...
            if (System.currentTimeMillis() - banStart.get(a) < (banLength
                     .get(a)) * 1000
                &&
                banRoom.get(a).equals(requestedRoom))
            {
               changeAllowed = false;
            }
            else
      ...
      // formerRoom. In this case just add the user to MainHall do not remove it
      // from anywhere.
      ...
      for (int j = 0; j < currentRoomCount; j++)
      {
         ...
         if (requestedRoom.equals(""))
         {
            ...
            // If user is being deleted stop listening for incoming messages to
            // allow thread to end.
            this.active = false;
         ...
         else if (requestedRoomObject.getRoomIdentity().equals(requestedRoom) &&
                  changeAllowed)
         {
            jsonObjServ.put("roomid", requestedRoom);

            // Update user list in respective rooms.
            requestedRoomObject.addUser(jsonObjRec.get("identity").toString());
            ...
            // To allow a user to kick user or delete room, need to allow this
      ...
      else
      {
         ...
         for (int k = 0; k < clientCount; k++)
         {
            if (ChatServerMain.getUser(k).getRoom().equals(requestedRoom) ||
                ChatServerMain.getUser(k).getRoom().equals(formerRoom))
            {
            ...
            if (ChatServerMain.getUser(k).getIdentity()
                     .equals(jsonObjRec.get("identity").toString()) &&
                requestedRoomObject.getRoomIdentity().equals("MainHall") &&
                !room.equals(""))
            {
               ...
               jsonObjMain.put("identities", requestedRoomObject
                        .getRoomContents());

               // Send roomlist message.
               RoomList(ChatServerMain.getUser(k)
                        .getWriter());
            }
         }
   ...
   private synchronized void RoomContents(JSONObject jsonObjRec)
   {
      ...
      for (int i = 0; i < ChatServerRoom.getRoomCount(); i++)
      {
                  ...
                  .equals(jsonObjRec.get("roomid").toString()))
         {
            ...
            jsonObjServ.put("owner", ChatServerMain.getRoom(i).getOwnerId());
            roomExists = true;
            break;
         }
      }
   ...
   private synchronized void RoomList(BufferedWriter out)
   {
      ...
      for (int i = 0; i < ChatServerRoom.getRoomCount(); i++)
      {
         ...
         roomDetail.put("roomid", ChatServerMain.getRoom(i).getRoomIdentity());
   ...
   private synchronized void KickUser(JSONObject jsonObjRec)
   {
      ...
      for (int i = 0; i < ChatServerRoom.getRoomCount(); i++)
      {
             ...
             ChatServerMain.getRoom(i).getOwnerId().equals(identity))
         {
            ...
            jsonObjKick.put("identity", jsonObjRec.get("identity").toString());
            ...
            {
               if (ChatServerMain.getUser(k).getIdentity()
                        .equals(jsonObjRec.get("identity")
                                 .toString()))
               {
                  userChanging = k;
                  break;
            ...
            ChatServerMain.getUser(userChanging)
                     .addBanRoom(jsonObjRec.get("roomid").toString());
            ChatServerMain.getUser(userChanging)
                     .addBanStart(System.currentTimeMillis());
            ChatServerMain
                     .getUser(userChanging)
                     .addBanLength((Long) (jsonObjRec.get("time")));
   ...
   private synchronized void DeleteRoom(JSONObject jsonObjRec)
   {
      ...
      for (int i = 0; i < ChatServerRoom.getRoomCount(); i++)
      {
                      ...
                      .equals(this.identity))
         {
            ...
            // Call room deletion on this room. Won't clash with room emptying
                                           ...
                                           BufferedWriter out)
   {
      ...
      {
         // No action taken if message fails to send. Message will just be lost.
      }
